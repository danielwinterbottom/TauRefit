...
void MiniAODRefitVertexProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup){
...
#else	
	std::unique_ptr<VertexCollection> VertexCollection_out = std::auto_ptr<VertexCollection>(new VertexCollection);
#endif
	...
	reco::Vertex newPV(thePV); // initialized to the PV
